...
statepop <- us_casesdeaths %>% 
            ...
            distinct(location, state, county, population) %>%
            group_by(location) %>%
...
casesdeaths <-
        us_casesdeaths %>% 
            ...
            rename(state_old = state, state = location) %>%
...
ggsave("projections/covid19-highest-growth-usregion.pdf", width = 11, height = 8)
...
ggsave("projections/covid19-state_growth-usregion.pdf", width = 11, height = 8)
...
ggsave("projections/covid19-dailychange-usregion.pdf", width = 11, height = 8)