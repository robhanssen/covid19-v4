...
casesdeathsbylocation %>%
        filter(date > today() %m-% months(18)) %>%
        ggplot() +
        aes(date, casesper100k) +
        geom_line(color = "blue", linetype = "dotted") +
        geom_line(aes(y = rollmean(casesper100k,
                avdays,
                na.pad = TRUE
        )),
        size = 2,
        ...
        scale_y_continuous(
                limit = c(0, 400),
                breaks = c(0, 2, 5, 10, 20, 50, 100 * 1:10),
                sec.axis = sec_axis(~ . / correction, breaks = seq(0, 5, 1))
        ) +
        ...
        ggtitle(paste(
                selected_state, "daily cases and deaths with",
                avdays,
                "days average line"
        )) +
...
selected_state <- "Florida"
us_casesdeaths %>%
        filter(state == selected_state, county == "Alachua") %>%
        group_by(date) %>%
        summarize(
                population = sum(population),
                cases = sum(cases),
