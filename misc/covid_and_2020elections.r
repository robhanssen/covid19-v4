...
electionresults <- read_csv("sources/2020electionresults.csv") %>%
        mutate(trumpvictory = cut(
                per_gop, c(0, 0.1 * 1:10),
...
casesperelection <- us_casesdeaths %>%
        filter(date > election) %>%
...
casesperelection %>%
        ...
        geom_col() +
        labs(
                title = "What happens when virus response is politicized?",
...
casespercounty %>%
        mutate(counter = per_gop * population) %>%
        summarize(voters = sum(counter))
...
casesperelection <- us_casesdeaths %>%
        mutate(stage = ifelse(date < cuttoffdate, "Before July 2020", "After July 2020"), stage = factor(stage, levels = c("Before July 2020", "After July 2020"))) %>%
        inner_join(electionresults) %>%
...
deathsgraph3 <-
        ...
        labs(
                title = "COVID-19 death rate by country average vote for Trump",
                x = "Date",
                y = "Cumulative COVID-19 deaths per 100,000",
                caption = paste0("COVID-19 deaths until ", format(today(), format = "%b %d, %Y"))
        ) +
...
# elec_breaks <- pnorm(c(-5, -.5, -.15, .15, .5, 5))
...
covidbyelections <- us_casesdeaths %>%
        ...
        inner_join(electionresults) %>%
        select(-trumpvictory) %>%
        mutate(gop_vic = cut(per_gop, breaks = elec_breaks, labels = elec_labels)) %>%
        group_by(date, gop_vic) %>%
        summarize(
                cases = sum(cases),
                deaths = sum(deaths),
